...
int main(int argc, char** argv)
{
  if( argc != 5 )
  {
    cout<<"usage: pose_estimation_3d3d srcImage_1, srcImage_2, 1_depth 2_depth"<<endl;
    return 1;
  } 
      ...
      std::vector< DMatch >& matches)
{
  ...
  //-- 第二步根据角点位置计算BRIEF描述子
  descriptor -> compute( srcImage_1, keypoints_1, descriptors_1 );
  descriptor -> compute( srcImage_2, keypoints_2, descriptors_2 );
  
  //-- 第三步对两幅图中的BRIEF描述子进行匹配, 使用Hamming距离
  vector< DMatch > match;
  ...
  //当描述子之间的距离大于两倍最小距离时,即认为匹配有误,但有时候最小距离会非常小,设置一个经验值30作为下限
  for( int i = 0; i < descriptors_1.rows; i++ ){
